...
public class Wagenparkbeheer {
	...
	DBCollection collUnits = db.getCollection("Units");
	...
	public void test() {
		...
		for (DBObject result : output.results()) {
			...
			for (int m = 0; m < arraylist.size(); m++) {
				...
				System.out.println("{\"UnitId\": " + "\"" + unit + "\"" + ", \"Dag\": " + "\"" + arraylist.get(m) + "\""
						+ ", \"KM\": " + "\"" + Math.round(gereden) + "\"}");

			}
			System.out.println(
					"UnitId: " + unit + ", Totaal KM vanaf " + arraylist.get(0) + ": " + Math.round(geredenTotaal));
			System.out.println(
	...
	public void saveData() {
		...
		DBCollection collection = db.createCollection("r.u1", options);
